...
async function main(){
  ...
  // todo: 模拟每一步的结果 async simulate = (result, delay)=> new Promise(resolve => setTimeout(()=>resolve(result), delay))
  // todo: 模拟每一步的成本 - 转账0.001, 转币 0.1%
  // todo: 每次trade结束后，记录模拟结果到文件
  // todo: 重复这个过程24小时
  ...
  for (let trade of trades) {
    try{
      console.log('\n\n')
      let simulateBalance = 0.5
      await api.makeTrade({...trade, simulate: true, simulateBalance})
    } catch (error) {
